...
export const VerdictBadge: React.FC<Props> = ({ badge, size = 'md', showLabel = true }) => {
  const configs = {
    excelente: {
      ...
      sub: 'Solo Perfeito / Aprovado',
      color: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
      icon: Award,
    },
    razoavel: {
      label: 'Aprovado no CQ',
      sub: 'Padrão Fábrica LS',
      color: 'bg-blue-500/15 text-blue-400 border-blue-500/30',
      icon: Scale,
    },
    alerta: {
      label: 'Ruído no Diferencial',
      sub: 'Solo Fora do Compasso',
      color: 'bg-amber-500/15 text-amber-400 border-amber-500/30',
      icon: AlertCircle,
    },
    perigo: {
      label: 'Corda Mizinha Estourou',
      sub: 'Tomada de Força Travada',
      color: 'bg-orange-500/15 text-orange-400 border-orange-500/30',
      icon: AlertTriangle,
    },
    agiota: {
      label: 'YOU DIED: Agiota',
      sub: 'Hitkill / Motor LS Fundido',
